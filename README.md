...
| `EV\|REBOOT_DETECTED` | 设备曾关机/重启。**不依赖任何广播** —— 判据是单调钟 `elapsedRealtime` 被复位（只有重启会复位它），并自动撤销关机造成的误报崩溃 |
...
至于**为什么崩**，两天日志给出很强的相关性证据：**15 次崩溃里有 13 次（87%）**，
在崩溃前 90 秒内都出现了 Wi-Fi / 移动网络的承载剧烈变化 —— 最典型的一次是 Wi-Fi
每秒回调一次、连刷 13 条，紧接着蓝牙就掉线。这指向 **2.4GHz 无线共存冲突**
（Wi-Fi 与蓝牙共用射频前端）。要坐实还需厂商 HAL / 固件层日志，App 层看不到，所以这里只写「相关」不写「因果」。
...
另外关机改用**纯计算自证**（见上表 `REBOOT_DETECTED`），彻底不依赖广播。
