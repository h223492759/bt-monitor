...
- **`STUCK_OFF` 可能偶发误报**：它是在采样时发现「`btSet=1` 但适配器还在 `OFF`」就立刻记一行，
  **没有宽限期**。而系统写 `bluetooth_on=1` 与适配器真正开始 `TURNING_ON` 之间有天然延迟，
  采样恰好落进这个窗口就会多写一行。它只影响这个辅助事件，**不影响 `SUSPECT_CRASH` 主判据**。
