...
object LogStore {
    ...
    private fun header(): String = buildString {
        ...
        append("# ------------------------------------------------------------------\n")
        append("# 字段: bt=蓝牙适配器态(OFF/TURNING_ON/ON/TURNING_OFF) btSet=系统期望(1开/0关/2飞行模式压制)\n")
        ...
        append("#   EV|shutdown        收到系统关机/重启广播（实时；服务内动态注册），带 已当场撤销=N\n")
        append("#   EV|CLOCK_JUMP      墙钟被调整（NTP 校时/手动改时间）——**不是**重启\n")
        append("#   EV|COUNT_ADJUSTED  计数被回正（撤销关机误报等），带 旧值->新值，可审计。\n")
        append("#                      关机时它可能出现在两处 —— 紧跟 EV|shutdown（广播先到）\n")
        append("#                      或紧跟 EV|SUSPECT_CRASH（掉线先到）。两条通道互相独立、先后随机，\n")
        append("#                      两种顺序都是正常的，不是漏写或写重。\n")
